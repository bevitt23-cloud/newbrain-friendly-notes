...
const TRAIT_PROMPTS: Partial<Record<CognitiveTrait, string>> = {
  working_memory:
    "Do NOT generate paragraphs for instructional content. Break each multi-step concept into a 3-step numbered checklist using <ol><li>. Step 1 demonstrates the action with a fully worked example the student just reads (no effort required). Step 2 walks through the same type of action with the student, phrased as 'Now let's try this together — ...' or 'Watch what we do next — ...'. Step 3 is a do-it-yourself prompt phrased as 'Your turn — try this one:' followed by a problem for the student to attempt. Do NOT label the steps with meta-terms like 'I Do', 'We Do', 'You Do' — just write the actual instructions directly. Keep all sentences under 15 words. Bold the actionable verbs with <strong>.",
  ef_planning:
    "Do NOT generate paragraphs for instructional content. Break each multi-step concept into a 3-step numbered checklist using <ol><li>. Step 1 demonstrates the action with a fully worked example the student just reads. Step 2 walks through the same type of action with the student, phrased as 'Now let's try this together — ...'. Step 3 is a do-it-yourself prompt phrased as 'Your turn — try this one:' followed by a problem for the student to attempt. Do NOT label the steps with meta-terms like 'I Do', 'We Do', 'You Do' — just write the actual instructions directly. Keep all sentences under 15 words. Bold the actionable verbs with <strong>.",
  rsd:
  ...
  interest_based:
    "The user requires extreme novelty to engage executive function. Explain the target concept entirely through the lens of their registered hyper-fixation. Map the academic variables directly to elements of their interest. RULE OF MECHANICAL ALIGNMENT (CRITICAL GUARDRAIL): The underlying mechanics of the academic concept MUST logically match the rules of the hyper-fixation. Every analogy must be a mathematically sound 1-to-1 mapping where the cause-effect relationships, proportions, and constraints mirror each other. FALLBACK DECISION RULE: Before using the hyper-fixation analogy, internally check: (1) Does every variable in the concept have a clear counterpart in the interest? (2) Do the cause-effect relationships match direction and proportion? (3) Are there no contradictions where the analogy would teach something false? If ANY of these three checks fail, do NOT force the analogy. Instead, use a concrete real-world example involving physical objects the user can visualize (cooking, sports, cars, animals, weather). Do NOT announce the switch, do NOT apologize, do NOT mention the interest at all in that section — just teach the concept using the real-world example as if it were your first choice. Never force a broken analogy — accuracy is non-negotiable.",
  dyscalculia:
...
STEP-BY-STEP MATH RULE: When solving a math problem or demonstrating a calculation, output a <div class="math-stepper" data-total-steps="N"> container. Inside it, place each step as <div class="math-step" data-step="N"><div class="math-step-equation">equation here</div><div class="math-step-explain">detailed plain English explanation with a concrete real-world analogy for the operation</div></div>. Number steps sequentially starting at 1. NEVER skip steps or combine multiple operations into one step. Each step explanation must include a real-world analogy that grounds the abstract math in something physical. The UI will render these with cascading reveal.`,
  ...
  prioritization_fatigue:
    "EXPLICIT HIERARCHY: The user struggles with executive function and information prioritization. You must visually rank information by weight so they can instantly distinguish what to memorize from background context. Use this HTML structure — and do NOT emit any 'Core Concept:' / 'Context:' / 'Example:' text labels, the visual treatment alone does the work:\n- Wrap the main testable idea of each section in <p class=\"priority-primary\"><strong>[the idea stated as a full sentence]</strong></p>. This is the ONE thing the student must remember.\n- Wrap supporting narrative or explanation in <p class=\"priority-secondary\">[the supporting sentence]</p>. These add context but aren't the core.\n- Wrap illustrative examples or case details in <p class=\"priority-tertiary\"><em>[the example]</em></p>. These help the idea stick but aren't independently testable.\nEach section should have exactly ONE priority-primary paragraph at or near the top, followed by priority-secondary and priority-tertiary paragraphs beneath. NEVER use asterisks for bold — only HTML <strong> tags.",
  visual_mapper:
